...
if "成交量有沒有怪怪的" not in html or "volgapNormal" not in html or "volgapMissing" not in html:
    ...
    sys.exit(1)
if "A 組先看清單" not in html or "app.js?v=20260701m" not in html or "styles.css?v=20260701i" not in html:
...
if "detail-sections" not in css or "detail-section" not in css:
    ...
    sys.exit(1)
if "kline-tooltip" not in css or "kline-chart-wrap" not in css or "kline-toolbar" not in css or "kline-legend" not in css or "kline-panel" not in css or "kline-chart" not in css:
