...
if "次要篩選：量能狀態" not in html or "volgapNormal" not in html or "volgapMissing" not in html:
    ...
    sys.exit(1)
if "今日主清單" not in html or "次要清單：今年 A 組" not in html or "app.js?v=20260701w" not in html or "styles.css?v=20260701p" not in html:
...
if "A 組：先看這組" not in app or "最近有候選" not in app or "A 組平均勝出大盤" not in app:
    ...
    sys.exit(1)
if "displayPriorityLabel" not in app or "出訊號：" not in app or "今日主清單" not in app or "signal_group_id" not in app or "版本 ${r.alias_count" not in app:
...
if "subtype-summary" not in css or "subtype-card" not in css:
    ...
    sys.exit(1)
if "main-a-groups" not in css or "main-a-head" not in css or "secondary-panel" not in css or "compact-spec" not in css or "compact-subtype" not in css:
