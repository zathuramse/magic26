...
if "A 組：重點觀察" not in app or "最新資料日期" not in app or "A 組 20 日超額表現" not in app:
    ...
    sys.exit(1)
if "displayPriorityLabel" not in app or "20 日表現" not in app or "動能分組" not in app or "20 日均額" not in app:
    print("round25 batch3 candidate-card metric copy missing from app.js")
    sys.exit(1)
if "原始資料" not in app or "調整後資料" not in app or "待人工確認" not in app or "流動性不足" not in app:
...
if "detailSectionHtml" not in app or "基本資料" not in app or "價格與表現" not in app or "量能集中度" not in app or "風險檢查" not in app or "研究狀態" not in app:
    ...
    sys.exit(1)
if "第一段缺口" not in app or "20 日超額表現" not in app or "前5大成交量占比" not in app or "量能分類" not in app:
    print("round25 batch4 detail plain-language labels missing from app.js")
    sys.exit(1)
if "量能分類" not in app or "volgap_score_impact" not in app:
